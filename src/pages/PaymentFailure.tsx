...
const PaymentFailure = () => {
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      if (!token) {
        toast.error("Veuillez vous reconnecter pour reprendre le paiement.");
        navigate("/login");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-card rounded-2xl shadow-lg border border-border p-8 text-center space-y-6">
        ...
        <p className="text-muted-foreground">Votre réservation n’a pas été confirmée.</p>
        ...
        <div className="space-y-3">
          ...
          >
            {retrying ? (
              ...
              <>
                <RotateCcw className="w-4 h-4 mr-2" /> Réessayer le paiement
              </>
            )}
          </Button>
