  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {user && !showForm && (
          <Button variant="outline" size="sm" className="rounded-full" onClick={() => setShowForm(true)}>
            Laisser un avis
          </Button>
        )}
      </div>
      ...
      {showForm && (
        <div className="mb-6 p-4 rounded-xl border border-border bg-muted/30">
          ...
          <div className="flex gap-2">
            ...
            >
              {createReview.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : null}
              Publier
            </Button>
            <Button size="sm" variant="ghost" className="rounded-full" onClick={() => setShowForm(false)}>
              Annuler
            </Button>
          </div>
        </div>
        ...
        <div className="space-y-5">
          {reviews.map((review) => {
            ...
            const initials = profile
              ...
              : "?";
            const name = profile
              ? [profile.first_name, profile.last_name].filter(Boolean).join(" ") || "Voyageur"
              : "Voyageur";

            return (
              <div key={review.id} className="flex gap-4">
                <Avatar className="h-10 w-10 shrink-0">
                  <AvatarFallback className="bg-accent/10 text-accent text-sm">{initials}</AvatarFallback>
                </Avatar>
